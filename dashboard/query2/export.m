let
  Source = AzureDataExplorer.Contents("https://insights.playfab.com", "92D06", "['events.all']  #(tab)| where FullName_Namespace == ""title.92D06"" #(tab)| project-keep EventData, Timestamp, EventId #(tab)| extend EventName = EventData[""EventName""] #(tab)| extend State = EventData[""State""] #(tab)| extend Version = EventData[""Version""] #(tab)| project-keep EventName, State, Version, Timestamp, EventId #(tab)| where Version >= 9", [MaxRows = null, MaxSize = null, NoTruncate = null, AdditionalSetStatements = null]),
